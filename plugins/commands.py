...
# Ask Doubt on telegram @KingVJ01
    ...
    if not files_:
        ...
        try:
            ...
            title = '@Merchandis  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
            ...
            g = await msg.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                     [
                         InlineKeyboardButton(' Watch Online🖥️', callback_data=f'generate_stream_link:{file_id}')
                     ]
                    ]
                )
            )
            k = await msg.reply(f"<b><u> </u></b>\nThis Movie File/Video will be deleted in <b><u>{AUTO_DELETE} mins</u> 🫥 <i></b>(Due to Copyright Issues)</i></b>",quote=True)
            await asyncio.sleep(AUTO_DELETE_TIME)
            await msg.delete()
            await g.delete()
